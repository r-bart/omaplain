...
class EmptySampleTests(unittest.TestCase):
    ...
    def test_the_card_height_does_not_depend_on_the_example(self) -> None:
        ...
        # entero cada 2,6 segundos.
        source = COMPONENT.read_text(encoding="utf-8")
        self.assertIn("implicitHeight: Style.space(96)", source)
        for field in ("head", "spare", "tail"):
            self.assertNotIn(f"{field}: root.sample", source.replace("root.sample." + field, ""))
        self.assertNotIn("wrapMode", source, "el cuerpo no debe ajustar línea")
